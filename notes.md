...
> _Про разницу `display: inline-block;` vs. `display: flex;`_
...
> **div.wrapper/.container должен иметь одну задачу** — во избежание конфликта стилей центрирующие обёртки должны иметь лишь одну задачу.

- Ещё раз, доп. обёртки для крупных секций вёрстку не испортят, потому, что в этом случае будет минимальное кол-во дополнительных узлов и не нагружает DOM, не превышает критическое число и к штрафу в Google не приводит. Совсем другое дело, когда доп. обёртки создаются на маленьких однотипных элементах, которых очень много. Особенно это касается записей блога, карточек товаров и т.п.

> **Адаптив меньше <576px**

- На Западе (например в Bootstrap и подобных фреймворках) в общем-то обычно не принято заморачиваться над адаптивом разрешений <576px и можно вполне сэкономить на них время.

> **Иерархия отступов**

- Наружные отступы между карточками обычно должны быть больше, чем отступы внутри каждой карточки.

> **Придерживаться одного стиля выравнивания в адаптиве**

- Не должно быть, чтобы один блок выровнен по центру, а другие по левому краю.

> **Использование rem для адаптива**

- Есть интересный подход использования величины rem: мы можем указать html свойство `font-size: 10px;` (и также указать body это же свойство, но 16px), после чего мы указываем размеры шрифтов и отступов в rem, а затем меняем в медиа-запросах только само свойство `font-size` у html и отступы и шрифты, указанные в rem величинах изменяются в величине соответственно. (Наиболее актуальным это является для самых крупных заголовков и отступов. Не нужно увлекаться.)

> **Когда ставить секции/блоку padding, а когда margin?**

- Если есть фон, то однозначно `padding`, иначе `margin`.

> **Когда мы видим, что макет сделан по сетке**

- Когда мы видим, что макет сделан по сетке, то ширины блокам можно указывать следующим образом, например сетка у нас стандарт в 12 колонок, а нам нужно, чтобы блок занимал 5 колонок то запись может быть такой (где у нас 15px расстояние между колонками): `width: calc(100% / 12 * 5 - 30px);`

> **Атрибут `aria-hidden="true"`**

- Для улучшения доступности полезно использовать в иконках, используемых только для украшения и не несущих логической нагрузки на сайте, например `<i class="fa fa-arrow" aria-hidden="true"></i>`.

> **Декоративные элементы при помощи `content`**

- Если у нас есть множество маленьких иконок или каких-то значков на сайте, то выгоднее будет их вставлять через `::before { content: url('...path...') }`.

> **Если браузер не поддерживает webP**

- Для контентных картинок всё просто: <picture><source><img></picture>. А для фоновых картинок либо ужимаем сайтом tinypng, либо можно написать маленький JS-код, который будет проверять поддержку браузером webP, и если проверка не пройдена добавлять класс, который уже используем добавляя элементу jpg/png картинку на фон.
